...
func (privateList *PrivateChatList) setNotificationCount(count int) {
	if count == 0 {
		privateList.internalTreeView.SetTitle("Özel Mesajlar")
	} else {
		privateList.internalTreeView.SetTitle(fmt.Sprintf("Özel Mesajlar[%s](%d)", tviewutil.ColorToHex(config.GetTheme().AttentionColor), count))
	}
}
