...
func NewLogin(app *tview.Application, configDir string) *Login {
	login := &Login{
		...
		tokenInputMaskRune:      '*',
		loginTypeTokenButton:    tview.NewButton("Token ile gir"),
		loginTypePasswordButton: tview.NewButton("E-Mail ile gir"),
	...
	splashScreen.SetText(tviewutil.Escape(unColoredTitle + "\n\nHesap bilgilerin yeri: " + configDir + "\n"))

	yapildi := tview.NewTextView()
	yapildi.SetTextAlign(tview.AlignRight)
	yapildi.SetText("hüseyinabi tarafından yapıldı")
	yapildi.SetTextColor(tcell.Color101)
	login.AddItem(yapildi, 2, 0, false)
	...
	login.usernameInput.SetTitle("Mail")
	configureInputComponent(login.passwordInput)
	login.passwordInput.SetTitle("Şifre")
	login.passwordInput.SetMaskCharacter('#')
	configureInputComponent(login.tfaTokenInput)
	login.tfaTokenInput.SetTitle("İki faktörlü doğrulama kodu")
...
func (login *Login) attemptLogin() {
	...
	login.messageText.SetText("Giriş yapılmaya çalışıyor...")

	switch login.loginType {
	case None:
		panic("Giriş yapılma yöntemi boştu.")
	case Token:
	...
	case Password:
		...
		if input != "" {
			...
			if parseError != nil {
				login.sessionChannel <- &loginAttempt{nil, errors.New("İki faktörlü doğrulama kodu yanlış.\n\nDoğru örnek: 564 231")}
			}
		}
...
func (login *Login) showLoginTypeChoice() {
	...
	login.app.SetFocus(login.loginTypeTokenButton)
	login.messageText.SetText("Lütfen bir giriş yöntemi seçin.\n\nBot ile giriş yapmak için token yerini kullanınız.")
}

func (login *Login) showPasswordLogin() {
	login.showView(login.passwordInputView, 9)
	login.app.SetFocus(login.usernameInput)
	login.messageText.SetText("Lütfen e-mail ve şifrenizi girin.\n\nİki faktörlü doğrulama kodunu etkinleştirmediyseniz,doğrulama kodu yerini boş bırakabilirsiniz.\nBİLGİ:@ işaretini koymak için CTRL+Q tuşuna basın.\nUygulamanın kendisi maalesef direkt desteklemiyordu.")
}

func (login *Login) showTokenLogin() {
	login.showView(login.tokenInputView, 3)
	login.app.SetFocus(login.tokenInput)
	login.messageText.SetText("Token kullanırken bot ile girmek istiyorsanız giriş yerine tokenden önce 'Bot ' yazın.\n\nNasıl tokeninizi almak için:\nhttps://github.com/Bios-Marcel/cordless/wiki/Retrieving-your-token \n\nToken yeri is normal olarak gizlenmiştir, açmak için Ctrl + R ye basın.")
}
