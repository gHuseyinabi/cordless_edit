...
func NewGuildList(guilds []*discordgo.Guild) *GuildList {
	...
	guildList.SetTitle("Sunucular").SetTitleAlign(tview.AlignLeft)
...
func (g *GuildList) setNotificationCount(count int) {
	if count == 0 {
		g.SetTitle("Sunucular")
	} else {
		g.SetTitle(fmt.Sprintf("Sunucular[%s](%d)", tviewutil.ColorToHex(config.GetTheme().AttentionColor), count))
	}
}
