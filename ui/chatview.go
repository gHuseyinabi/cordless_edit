...
func (chatView *ChatView) SetTitle(text string) {
	chatView.internalTextView.SetTitle(text + "napim")
}
...
func (chatView *ChatView) formatMessageText(message *discordgo.Message) string {
	if message.Type == discordgo.MessageTypeDefault {
		...
		return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "]sunucuya katıldı."
	} else if message.Type == discordgo.MessageTypeCall {
		return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "]bir aramayı başlattı."
	} else if message.Type == discordgo.MessageTypeChannelIconChange {
		return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "]sunucu resmini değiştirdi."
	} else if message.Type == discordgo.MessageTypeChannelNameChange {
		return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "]sunucu ismini " + message.Content + " e çevirdi."
	} else if message.Type == discordgo.MessageTypeChannelPinnedMessage {
		return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "]bir mesaj sabitledi."
	} else if message.Type == discordgo.MessageTypeRecipientAdd {
		return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "]gruba " + message.Mentions[0].Username + " kişisini ekledi."
	} else if message.Type == discordgo.MessageTypeRecipientRemove {
		return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "] " + message.Mentions[0].Username + " kullanıcısını banladı."
	} else if message.Type == discordgo.MessageTypeChannelFollowAdd {
		return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "]'" + message.Content + "' kanala katıldı."
		...
		return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "]svyi boostladı *vuhuuuuuuuuu*."
	}

	//TODO Support boost messages; Would be handy to see what they look like first.

	//Might happen when there are unsupported types.
	return "[" + tviewutil.ColorToHex(config.GetTheme().InfoMessageColor) + "]mesajı görüntülenemedi :("
}
