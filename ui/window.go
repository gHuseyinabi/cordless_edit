...
func NewWindow(doRestart chan bool, app *tview.Application, session *discordgo.Session, readyEvent *discordgo.Ready) (*Window, error) {
	...
	if config.Current.ShowBottomBar {
		...
		bottomBar.AddItem(fmt.Sprintf("'%s' Olarak giriş yapıldı.", tviewutil.Escape(session.State.User.Username)))
		bottomBar.AddItem(fmt.Sprintf("Görüntü / Kısayolları değiştir: %s", shortcutdialog.EventToString(shortcutsDialogShortcut)))
...
func getWelcomeText() string {
	...
	[red]Özellik:Şifreli mesaj için mesajın önüne [blue]'/enc '[red] koyun.
	[red]Yeni Özellik!Mesajlarınızı spamlamak için [blue]/spam [yellow]'mesaj'[red] yazın ve chate 10 kere mesajınızı atsın!
	[green]
	                          ...ooooo.         ..---##o
                          .--^""#########o ..o--"  .o#####
        .."-.         ..-^"  .o###########^"    .o########
...
func proceedMesages(window *Window, message *discordgo.Message) {
	...
	if strings.HasPrefix(cont, "ENC") {
		message.Content = string(
			util.Decrypt(util.DecryptBase64(cont[3:]), "golang_malclub_encryption_key111")) + "(Şifrelendi)"
	} //old-version
